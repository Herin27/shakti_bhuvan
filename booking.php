...
<body>
    ...
    <div class="booking-container">
        <div class="summary-card">
            ...
            <table class="calculation-table">
                <tr>
                    ...
                    <td class="value">₹<span
                            id="display_room_rate"><?php echo htmlspecialchars(number_format($room['discount_price'])); ?></span>
                    </td>
                </tr>
                <tr>
        ...
        <div class="booking-form">
            ...
            <form id="bookingForm" action="process_booking.php" method="POST">
                ...
                <input type="hidden" id="extra_bed_price_val"
                    value="<?php echo htmlspecialchars($room['extra_bed_price']); ?>">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select id="extra_bed" name="extra_bed" onchange="toggleExtraBedCount()">
                            ...
                            <option value="1">Yes
                                (₹<?php echo htmlspecialchars(number_format($room['extra_bed_price'])); ?> / night)
                            </option>
                        </select>
                    </div>
    ...
    // Availability ચેક કરવાનું ફંક્શન
    function validateAvailability() {
        const checkin = checkinInput.value;
        const checkout = checkoutInput.value;
        const roomId = <?php echo $room_id; ?>; // PHP માંથી રૂમ ID લો

        if (checkin && checkout && checkin < checkout) {
            fetch(`check_availability_api.php?checkin=${checkin}&checkout=${checkout}&room_id=${roomId}`)
    ...
    // જ્યારે ચેક-ઈન કે ચેક-આઉટ તારીખ બદલાય ત્યારે ચેક કરો
    checkinInput.addEventListener('change', validateAvailability);
    checkoutInput.addEventListener('change', validateAvailability);

    function checkAvailability(checkin, checkout, roomId) {
        fetch(`check_availability_api.php?checkin=${checkin}&checkout=${checkout}&room_id=${roomId}`)
            .then(response => response.json())
            .then(data => {
                if (data.available <= 0) {
                    alert("This room type is fully booked for these dates!");
    ...
    // જ્યારે તારીખ બદલાય ત્યારે આ ફંક્શન કોલ કરો
    checkinInput.addEventListener('change', () => {
        if (checkinInput.value && checkoutInput.value)
            checkAvailability(checkinInput.value, checkoutInput.value, <?php echo $room_id; ?>);
    });
